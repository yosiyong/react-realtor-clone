...
export default function EditListing() {
  ...
  const params = useParams()
    ...
    useEffect(()=>{
        if(listing && listing.userRef !== auth.currentUser.uid) {
            toast.error(t("You can't edit this listing"));
            navigate("/")
        }
    },[auth.currentUser.uid, listing, navigate]);

  //遷移されて物件IDが変わった場合、物件情報取得
  useEffect(()=>{
    setLoading(true);
    ...
    async function fetchListing() {
        ...
        if (docSnap.exists()) {
            ...
            navigate("/");
            toast.error(t("Listing does not exist"));
        }
    }
  ...
  async function onSubmit(e) {
    ...
    setLoading(false);
    toast.success(t("Listing Edited"));
...
return (
  <main className="max-w-md px-2 mx-auto">
    <h1 className="text-3xl text-center mt-6 font-bold">{t('Edit Listing')}</h1>
